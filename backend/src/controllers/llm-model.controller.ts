...
export class LlmModelController {
  ...
  static async getModelById(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: 'Erreur lors de la récupération du modèle' });
    }
  }
  ...
  static async updateModel(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: 'Erreur lors de la mise à jour du modèle' });
    }
  }
  ...
  static async deleteModel(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: 'Erreur lors de la suppression du modèle' });
    }
  }
}
